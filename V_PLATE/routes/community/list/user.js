//	커뮤니티 유저영상 모아보기	/community/list/user
...
router.get( '/' , function( req ,res ) {
	...
	let task = [
		...
		function( connection , callback ) {
			...
			let selectCommunity = 'SELECT * FROM community C , mymedia M , user U , template T ' +
			'WHERE C.mymedia_id = M.mymedia_id AND M.user_email = U.user_email AND T.template_id = M.template_id AND U.user_email = ? ' +
			'ORDER BY community_uploadtime DESC' ;

			connection.query( selectCommunity , useremail , function( err , result ) {
				if( err ) {
					res.status(500).send( {
						status : "fail" ,
						msg : "internal server err"
			...
			connection.query( infoBookmark , email , function( err , result ) {
				if( err ) {
					...
					for( let i = 0 ; i < object.length ; i++ ) {
						...
						let data = {
							communiyt_id : object[i].community_id ,
		...
		function( list , callback ) {
			...
			res.status(200).send({
				...
				msg: "successfully get community user lists"
			});
			callback ( null , "successfully get community user lists" ) ;
		}
	] ;
